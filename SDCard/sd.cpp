...
namespace SD
{
	...
	SDCard::SDCard(const uint8_t& spiChannel, Chimera::GPIO::Port port, uint8_t pin)
	{
		STATUS = STA_NOINIT;
		...
		spiTXRXWakeup = xSemaphoreCreateBinary();
	...
	{
		/* Put the SD card in SPI mode by clocking in at LEAST 74 SCK cycles while the CS and MOSI pins are State::HIGH. */
		SD_IO_CSState(1);
	...
	void SDCard::SD_IO_CSState(uint8_t state)
	{
		if (state == 1)
			...
			ssPin->write(State::LOW);
	}

	void SDCard::SD_IO_WriteReadData(uint8_t *DataIn, uint8_t *DataOut, uint16_t DataLength)
	{
		spi->write(DataIn, DataOut, DataLength, false);

		/* Don't wake up the thread until transmission is complete */
		#ifdef USING_FREERTOS
		//xSemaphoreTake(spiTXRXWakeup, portMAX_DELAY);
		#endif 
	}
	...
	void SDCard::SPI_Init(void)
	{
		...
		spi->attachThreadTrigger(TXRX_COMPLETE, &spiTXRXWakeup);
		#endif 
	...
	SDCard::SD_CmdAnswer SDCard::SD_SendCmd(uint8_t Cmd, uint32_t Arg, uint8_t Crc, uint8_t Answer)
	{
		uint8_t frame[SD_CMD_LENGTH], frameout[SD_CMD_LENGTH];
		SD_CmdAnswer retr = { 0xFF, 0xFF , 0xFF, 0xFF, 0xFF };
