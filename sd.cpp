...
/* Private variables ---------------------------------------------------------*/
__IO uint8_t SD_CARD_STATUS = SD_NOT_PRESENT;
uint16_t FLAG_SDHC = 0; /* 0: Standard Capacity, 1: High Capacity */


/*------------------------------ 
 * SD Card Class
 *-----------------------------*/
SDCard::SDCard(SPIClass_sPtr spi_instance)
{
	STATUS = STA_NOINIT;
	spi = spi_instance;

	/*-------------------------------------------------------------------------------
	 * Bind specific implementations of the functions used by FatFS to
	 * access the SD card structure. Normally these functions come from 
	 * sd_diskio.cpp. Bind is needed due to being able to pass in pointers to
	 * various SPI instances, which causes the implementation details to change.
	 *------------------------------------------------------------------------------*/
	Driver.disk_initialize = boost::bind<DSTATUS>(&SDCard::SD_Initialize, this, _1);
	Driver.disk_status = boost::bind<DSTATUS>(&SDCard::SD_Status, this, _1);
	Driver.disk_read = boost::bind<DRESULT>(&SDCard::SD_Read, this, _1, _2, _3, _4);

	#if _USE_WRITE == 1
	Driver.disk_write = boost::bind<DRESULT>(&SDCard::SD_Write, this, _1, _2, _3, _4);
	#endif

	#if _USE_IOCTL == 1
	Driver.disk_ioctl = boost::bind<DRESULT>(&SDCard::SD_ioctl, this, _1, _2, _3);
	#endif

	/* Force the SPI instance to have the proper settings. The SPI instance begin() function
	 * MUST have already been called and any external slave selects initialized. */
	SPIx_ReInit();
}
...
/* Public Functions */
FRESULT SDCard::initialize(bool formatDrive, uint32_t formatOptions)
{
	FATFS_LinkDriver(&Driver, SDPath);
	
	if (formatDrive)
	{
		FRESULT error = FR_OK;
		error = format(formatOptions);
...
FRESULT SDCard::format(uint32_t FA_format_options)
{
	return f_mkfs((TCHAR const*)SDPath, FA_format_options, 0, workBuffer, sizeof(workBuffer));
}

FRESULT SDCard::fopen(const char* filename, uint32_t FA_open_options, FIL& file_ptr)
{
	return f_open(&file_ptr, filename, FA_open_options);
}

FRESULT SDCard::fclose(FIL& file_ptr)
{
	return f_close(&file_ptr);
}

FRESULT SDCard::read(FIL& file_ptr, uint8_t* data_out, size_t length, uint32_t& bytesRead)
{
	return f_read(&file_ptr, data_out, length, (UINT*)bytesRead);
}

FRESULT SDCard::write(FIL& file_ptr, uint8_t* data_in, size_t length, uint32_t& bytesWritten)
{
	return f_write(&file_ptr, data_in, length, (UINT*)bytesWritten);
}

/* Private Functions */
DSTATUS SDCard::SD_Initialize(BYTE lun)
{
	STATUS = STA_NOINIT;
...
DSTATUS SDCard::SD_Status(BYTE lun)
{
	STATUS = STA_NOINIT;
...
DRESULT SDCard::SD_Read(BYTE lun, BYTE *buff, DWORD sector, UINT count)
{
...
#if _USE_WRITE == 1
DRESULT SDCard::SD_Write(BYTE lun, const BYTE *buff, DWORD sector, UINT count)
{
	DRESULT res = RES_ERROR;

	if (BSP_SD_WriteBlocks((uint32_t*)buff, (uint32_t)(sector), count, 0x10000) == MSD_OK)
	{
...
#if _USE_IOCTL == 1
DRESULT SDCard::SD_ioctl(BYTE lun, BYTE cmd, void *buff)
{
	DRESULT res = RES_ERROR;
...
void SDCard::SD_IO_Init(void)
{
	/* If needed, GPIO config can go here. For now, this is
		taken care of by the SPI class. */


	/* Put the SD card in SPI mode by clocking in at LEAST 74 SCK cycles
	   while the CS and MOSI pins are high. */
	spi->setSSMode(ThorDef::SPI::SS_MANUAL_CONTROL);
	spi->writeSS(HIGH);
...
void SDCard::SD_IO_CSState(uint8_t state)
{
	if (state == 1)
		spi->writeSS(HIGH);
	else
...
void SDCard::SD_IO_WriteReadData(uint8_t *DataIn, uint8_t *DataOut, uint16_t DataLength)
{
	/* Prevent the freeRTOS scheduler from interrupting communications. SPI is forced 
	 * into blocking mode due to the FatFS structure and cannot handle asynchronous
	 * interruptions. */
	#ifdef USING_FREERTOS
	taskENTER_CRITICAL();
	#endif 
	
	spi->write(DataIn, DataOut, DataLength, ThorDef::SPI::NO_OPTIONS);
...
uint8_t SDCard::SD_IO_WriteByte(uint8_t Data)
{
	uint8_t tmp;
...
void SDCard::SPIx_ReInit(void)
{
	SPI_InitTypeDef tempInit = spi->getSettings();

	/* Critical settings */
	tempInit.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_2; //Max is around 20MHz clock
	tempInit.Direction = SPI_DIRECTION_2LINES;
	tempInit.CLKPhase = SPI_PHASE_1EDGE;
	tempInit.CLKPolarity = SPI_POLARITY_LOW;
	tempInit.CRCCalculation = SPI_CRCCALCULATION_DISABLE;
	tempInit.CRCPolynomial = 7;
	tempInit.DataSize = SPI_DATASIZE_8BIT;
	tempInit.FirstBit = SPI_FIRSTBIT_MSB;
	tempInit.NSS = SPI_NSS_SOFT;

	spi->attachSettings(tempInit);
	spi->reInitialize();

	/* Currently using blocking mode is a limitation of the FatFS code and not SPI */
	spi->setTxModeBlock();
	spi->setRxModeBlock();
}
...
*/
uint8_t SDCard::BSP_SD_Init(void)
{
	/* Configure IO functionalities for SD pin */
	SD_IO_Init();
	SD_CARD_STATUS = SD_PRESENT;
...
* @retval SD status
*/
uint8_t SDCard::BSP_SD_ReadBlocks(uint32_t *pData, uint32_t ReadAddr, uint32_t NumOfBlocks, uint32_t Timeout)
{
	uint32_t offset = 0;
	...
	ptr = (uint8_t*)pvPortMalloc(sizeof(uint8_t)*BlockSize);
...
* @retval SD status
*/
uint8_t SDCard::BSP_SD_WriteBlocks(uint32_t *pData, uint32_t WriteAddr, uint32_t NumOfBlocks, uint32_t Timeout)
{
	uint32_t offset = 0;
	uint8_t retr = BSP_SD_ERROR;
	uint8_t *ptr = NULL;
...
* @retval SD status
*/
uint8_t SDCard::BSP_SD_Erase(uint32_t StartAddr, uint32_t EndAddr)
{
	uint8_t retr = BSP_SD_ERROR;
	SD_CmdAnswer_typedef response;
	uint16_t BlockSize = 512;
...
*/
uint8_t SDCard::BSP_SD_GetCardState(void)
{
...
*         - MSD_OK: Sequence succeed
*/
uint8_t SDCard::BSP_SD_GetCardInfo(SD_CardInfo *pCardInfo)
{
	uint8_t status;
...
* @retval SD status
*/
uint8_t SDCard::SD_GetCSDRegister(SD_CSD* Csd)
{
	uint16_t counter = 0;
	uint8_t CSD_Tab[16];
	uint8_t retr = BSP_SD_ERROR;
...
* @retval SD status
*/
uint8_t SDCard::SD_GetCIDRegister(SD_CID* Cid)
{
	uint32_t counter = 0;
	uint8_t retr = BSP_SD_ERROR;
	uint8_t CID_Tab[16];
...
* @retval SD status
*/
SDCard::SD_CmdAnswer_typedef SDCard::SD_SendCmd(uint8_t Cmd, uint32_t Arg, uint8_t Crc, uint8_t Answer)
{
...
*/
uint8_t SDCard::SD_GetDataResponse(void)
{
...
* @retval SD status
*/
uint8_t SDCard::SD_GoIdleState(void)
{
	SD_CmdAnswer_typedef response;
...
*/
uint8_t SDCard::SD_ReadData(void)
{
...
*/
uint8_t SDCard::SD_WaitData(uint8_t data)
{
